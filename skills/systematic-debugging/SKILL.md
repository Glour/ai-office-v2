...
3. **Что изменилось?**
   ...
   - Обновление OpenClaw? (`openclaw --version`)
...
5. **Если 3+ фикса не сработали** - СТОП. Проблема архитектурная. Обсуди с {{OWNER_NAME}}.
...
|----------|---------------------|
| Крон не запустился | `openclaw cron list` → consecutiveErrors, lastStatus |
| Gateway не отвечает | `openclaw gateway status`, порт 18789 |
| Скрипт не найден | Путь, chmod +x, shebang |
| Docker упал | `docker ps -a`, `docker logs <name>` |
| Память не ищет | `sqlite3 ~/.openclaw/memory/main.sqlite "SELECT count(*) FROM chunks;"` |
| Бот молчит | message tool → проверь to, channel |
| После обновления | `bash scripts/post-update.sh`, `openclaw doctor` |
